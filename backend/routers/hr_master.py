"""Plant-scoped employee master and salary structure.

PR34 reuses authentication users for identity and stores only employment/HR
metadata plus versioned salary structures. Salary structures are advisory input
for payroll preparation; they never auto-create or auto-pay payroll records.
"""
...
async def save_employee_profile(plant_id: str, user_id: str, body: EmployeeProfileBody, ctx: dict = Depends(owner_only)):
    ...
    user = await _employee(plant_id, user_id)
    if body.join_date > _utcnow().date() + __import__("datetime").timedelta(days=365):
        raise HTTPException(422, "Join date is unreasonably far in the future")
    now = _utcnow()
    payload = {**body.model_dump(), "plant_id": plant_id, "user_id": user_id, "user_name": user.get("name") or "Staff", "role": user.get("primary_role"), "updated_by": ctx["user_id"], "updated_at": now}
    try:
        await employee_profiles.update_one({"plant_id": plant_id, "user_id": user_id}, {"$set": payload, "$setOnInsert": {"created_at": now}}, upsert=True)
...
async def save_salary_structure(plant_id: str, user_id: str, body: SalaryStructureBody, ctx: dict = Depends(owner_only)):
    ...
    now = _utcnow()
    payload = {**body.model_dump(), "effective_from": body.effective_from.isoformat(), "plant_id": plant_id, "user_id": user_id, "gross_amount": totals["gross"], "fixed_allowances": totals["allowances"], "fixed_deductions_total": totals["deductions"], "net_before_overtime": totals["net_before_overtime"], "updated_by": ctx["user_id"], "updated_at": now}
    await salary_structures.update_one({"plant_id": plant_id, "user_id": user_id, "effective_from": body.effective_from.isoformat()}, {"$set": payload, "$setOnInsert": {"created_at": now}}, upsert=True)
    salary = await salary_structures.find_one({"plant_id": plant_id, "user_id": user_id, "effective_from": body.effective_from.isoformat()})
    await write_audit(ctx["user_id"], "hr.employee.salary.update", "hr_salary_structure", str((salary or {}).get("_id")), {"plant_id": plant_id, "user_id": user_id, "effective_from": body.effective_from.isoformat(), "gross": totals["gross"]})
