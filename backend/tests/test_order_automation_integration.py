...
def isolated_automation_collections(monkeypatch):
    database = order_automation.db.client[
        f"{order_automation.db.name}_pytest_automation_{uuid4().hex}"
    ]
    collections = {
        "orders": database.orders,
        "order_automation_events": database.order_automation_events,
        "order_automation_actions": database.order_automation_actions,
        "order_automation_attempts": database.order_automation_attempts,
    }
    for name, collection in collections.items():
        monkeypatch.setattr(order_automation, name, collection)

    try:
        yield collections
    finally:
        database.client.delegate.drop_database(database.name)
...
def test_every_canonical_status_reaches_provider_mock_and_ack(
    monkeypatch, isolated_automation_collections
):
...
async def _exercise_all_statuses(collections):
    order_id = ObjectId()
    await collections["orders"].insert_one({
        "_id": order_id,
    ...
    for status in CANONICAL_STATUSES:
        ...
        completed = await collections["order_automation_events"].find_one(
            {"_id": claimed["_id"]}
        )
        assert completed["state"] == STATE_COMPLETED
        action = await collections["order_automation_actions"].find_one({
    ...
    assert await collections["order_automation_events"].count_documents(
        {"state": STATE_COMPLETED}
    ) == len(CANONICAL_STATUSES)
    assert await collections["order_automation_attempts"].count_documents(
        {"action": "COMPLETED"}
    ) == len(CANONICAL_STATUSES)
