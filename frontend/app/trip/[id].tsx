...
type Trip = { id: string; order_number: string; status: string; grade: string; quantity: number; tm_number: string; site_name: string; site_address?: string };
type Detail = { trip: Trip; site_address?: string; pod?: { receiver_name: string; delivered_quantity: number } | null };

const ACTIONS: Record<string, { path: string; label: string; icon: keyof typeof Ionicons.glyphMap } | undefined> = {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.surface }}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.divider }]}>
        <Pressable testID="trip-back" onPress={() => router.back()} style={[styles.iconBtn, { borderColor: colors.border }]}>
      ...
      {error && !data ? (
        ...
        <View style={{ padding: spacing.lg }}><Skeleton height={300} style={{ borderRadius: radius.lg }} /></View>
      ) : t ? (
            ...
            <View style={styles.stages}>
              {STAGES.map((s, i) => (
                <React.Fragment key={s}>
                  <View style={[styles.dot, { backgroundColor: i <= stageIdx ? colors.brand : colors.surfaceTertiary }]} />
                  {i < STAGES.length - 1 ? <View style={[styles.bar, { backgroundColor: i < stageIdx ? colors.brand : colors.surfaceTertiary }]} /> : null}
                </React.Fragment>
              ))}
            </View>
            <View style={styles.rowBetween}>
          ...
          {t.status === "DELIVERED" ? (
            <Card style={{ flexDirection: "row", gap: spacing.sm, alignItems: "center", backgroundColor: colors.success + "1A", borderColor: colors.success + "55" }}>
              ...
              <AppText style={{ flex: 1, fontFamily: fonts.medium, color: colors.onSurface }}>
                Delivered{data?.pod ? ` · received by ${data.pod.receiver_name} (${data.pod.delivered_quantity} m³)` : ""}
              </AppText>
            </Card>
          ) : (
            <View style={{ gap: spacing.sm }}>
              <Button testID="trip-navigate" label="Open Navigation" variant="secondary" onPress={() => toast("Turn-by-turn opens with the Maps key", "info")} icon={<Ionicons name="navigate-outline" size={18} color={colors.onSurface} />} />
              {action ? (
                <Button testID="trip-advance" label={action.label} loading={busy} onPress={() => advance(action.path, `${action.label} done`)} icon={<Ionicons name={action.icon} size={18} color={colors.onBrand} />} />
              ) : t.status === "UNLOADING" || t.status === "POD_PENDING" ? (
                <Button testID="trip-pod" label="Complete Delivery (POD)" onPress={() => router.push(`/pod/${id}` as any)} icon={<Ionicons name="clipboard-outline" size={18} color={colors.onBrand} />} />
              ) : null}
            </View>
          )}
        </ScrollView>
