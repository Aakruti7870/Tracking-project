...
type Shift = { id: string; name: string; start_time: string; end_time: string; grace_minutes: number };
...
type OwnerPlant = {
  ...
  geofence_exceptions: ExceptionRow[];
  employees: Employee[];
};
        ...
        {ownerMode && plant ? (
          <>
            <Card style={{ gap: spacing.md }}>
              ...
              <Input label="Plant latitude" value={lat} onChangeText={setLat} keyboardType="decimal-pad" placeholder="19.000000" />
              <Input label="Plant longitude" value={lng} onChangeText={setLng} keyboardType="decimal-pad" placeholder="73.000000" />
              <Input label="Allowed radius (metres)" value={radius} onChangeText={setRadius} keyboardType="numeric" />
              <Input label="Timezone" value={timezoneName} onChangeText={setTimezoneName} placeholder="Asia/Kolkata" />
              <Button disabled={busy || !lat || !lng} onPress={() => run(() => apiPut(`/workforce-rules/plants/${plant.plant_id}/settings`, token!, { center_lat: Number(lat), center_lng: Number(lng), radius_m: Number(radius || 250), timezone_name: timezoneName, enabled: true, max_accuracy_m: 250 }), "Attendance geofence saved")}>Save Geofence</Button>
            </Card>
            ...
            <Card style={{ gap: spacing.md }}>
              ...
              <View style={styles.wrap}>{plant.employees.map((e) => <Pressable key={e.user_id} onPress={() => setSelectedEmployee(e.user_id)} style={[styles.chip, { borderColor: selectedEmployee === e.user_id ? colors.brand : colors.border, backgroundColor: selectedEmployee === e.user_id ? colors.surfaceSecondary : colors.surface }]}><AppText>{e.name}</AppText></Pressable>)}</View>
              ...
              <Button disabled={busy || !selectedEmployee || (!weekOff && !selectedShift)} onPress={() => run(() => apiPut(`/workforce-rules/plants/${plant.plant_id}/roster/${rosterDate}/${selectedEmployee}`, token!, { shift_id: selectedShift, week_off: weekOff, note: null }), "Roster updated")}>Save Roster</Button>
            </Card>

            <Card style={{ gap: spacing.md }}>
              <AppText variant="heading">Pending Geofence Exceptions</AppText>
              {plant.geofence_exceptions.length === 0 ? <AppText variant="caption">No pending location exceptions.</AppText> : plant.geofence_exceptions.map((x) => <View key={x.id} style={styles.listRow}><View style={{ flex: 1 }}><AppText style={{ fontFamily: fonts.semibold }}>{x.user_name || "Staff"}</AppText><AppText variant="caption">{x.date} · In {x.check_in_geofence_status || "-"} · Out {x.check_out_geofence_status || "-"}</AppText></View><Button size="sm" disabled={busy} onPress={() => run(() => apiPost(`/workforce-rules/plants/${plant.plant_id}/attendance/${x.date}/${x.user_id}/geofence-review`, token!, { action: "ACCEPT", note: "Owner reviewed and accepted attendance evidence" }), "Exception accepted")}>Accept</Button></View>)}
            </Card>

            <Card style={{ gap: spacing.md }}>
              <AppText variant="heading">Attendance Corrections</AppText>
              {plant.pending_corrections.length === 0 ? <AppText variant="caption">No pending correction requests.</AppText> : plant.pending_corrections.map((c) => <View key={c.id} style={{ gap: spacing.sm }}><AppText style={{ fontFamily: fonts.semibold }}>{c.user_name || "Staff"} · {c.attendance_date}</AppText><AppText variant="caption">{c.reason}</AppText><View style={styles.twoCol}><Button size="sm" disabled={busy} onPress={() => run(() => apiPost(`/workforce-rules/plants/${plant.plant_id}/corrections/${c.id}/decision`, token!, { action: "APPROVE", note: "Approved by Plant Owner" }), "Correction approved")}>Approve</Button><Button size="sm" variant="secondary" disabled={busy} onPress={() => run(() => apiPost(`/workforce-rules/plants/${plant.plant_id}/corrections/${c.id}/decision`, token!, { action: "REJECT", note: "Rejected by Plant Owner" }), "Correction rejected")}>Reject</Button></View></View>)}
            </Card>
          </>
        ) : (
          <>
            ...
            <Card style={{ gap: spacing.md }}>
              ...
              <View style={styles.twoCol}><View style={{ flex: 1 }}><Input label="Punch in HH:MM" value={correctionIn} onChangeText={setCorrectionIn} placeholder="09:00" /></View><View style={{ flex: 1 }}><Input label="Punch out HH:MM" value={correctionOut} onChangeText={setCorrectionOut} placeholder="18:00" /></View></View>
              <Input label="Reason" value={correctionReason} onChangeText={setCorrectionReason} placeholder="Explain the correction" />
              <Button disabled={busy || correctionReason.trim().length < 5 || (!correctionIn && !correctionOut)} onPress={() => {
